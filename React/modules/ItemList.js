...
class ItemList extends React.Component{
    ...
    renderItem = ({item}) => (
        this.checkColor(this.state.props, item),
        item = {
            title: item.title,
            color: this.color,
            link: item.link,
            isNew: item.isNew,
            nav: this.state.props.navigation
        },
        <View style={styles.fullItem}>
            <View style={styles.chip} opacity={item.isNew}/>
            <View style={styles.item} backgroundColor={this.color} onMoveShouldSetResponder={() => true} onResponderRelease={() => {this.navigation(item)}}>
